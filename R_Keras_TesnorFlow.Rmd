...
There are currently at least three ways to use TensorFlow from R.
1. **Estimator API**: high-level interfaces to common models    
2. **Core API**: low-level interface to TensorFlow Computational Graph    
3. **Keras API**: high-level to TensorFlow neural networks    

In this session we will look briefly at the Estimator API and spend some time with the `Keras` interface.

### Estimator API
The [`tfestimators`](https://tensorflow.rstudio.com/tfestimators/) package on GitHub provides an R interface to the TensorFlow Estimators API that offers pre-built TensorFlow models including linear regression and classification models and deep learning neural nets. Other models such as KMeans and SVMs are expected soon.

![](tfestimators.png)

Look [here](https://www.edgarsdatalab.com/2017/10/22/intro-to-tensorflow-in-r/#fullcode) for an example running a linear classifier model directly from R.
